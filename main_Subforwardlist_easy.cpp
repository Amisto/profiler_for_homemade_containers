...
//----------- Test 001 Straight pop_forward
    ...
    for (int i = 0; i < n; i++)
    {
        if (sfl->pop_forward() != test_sequence[i])
        {
...
//----------- Test 002 Straight push_forward
    ...
    for (int i = 0; i < n; i++)
    {
        sfl->push_forward(test_sequence[i]);
    }
    finish = get_time();
...
//----------- Test 003 Straight pop_back
    ...
    for (int i = 0; i < n; i++)
    {
        if (sfl->pop_back() != test_sequence[i])
        {
            cout <<endl <<"--- !!! Failed push/pop consistency !!! ---" << endl;
            return 0;
        }
...
//----------- Test 004 Random push_where
    ...
    for (int i = 0; i < n; i++)
    {
        sfl->push_where(push_sequence[i], test_sequence[i]);
    }
    finish = get_time();
...
//----------- Test 005 Random erase_where
    ...
    for (int i = 0; i < 2 * n; i++)
    {
        sfl->push_back(test_sequence[i % n]);
    }

    start = get_time();
    for (int i = 0; i < n; i++)
    {
        sfl->erase_where(pop_sequence[i]);
    }
    finish = get_time();
